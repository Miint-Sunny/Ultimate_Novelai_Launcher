      ...
      <div className="p-4 border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="grid grid-cols-2 gap-1 bg-black/30 rounded-lg p-1 border border-gray-800 relative w-[160px] h-9">
            <div
              className="absolute inset-y-1 bg-indigo-600 rounded-md shadow transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)]"
              style={{
            ...
            <button
              ...
              className={`relative z-10 px-3 h-full flex items-center justify-center text-xs font-bold rounded-md transition-colors ${
                activeSource === 'public' ? 'text-white' : 'text-gray-400 hover:text-gray-200'
              }`}
            >
              公共
            </button>
            <button
              ...
              className={`relative z-10 px-3 h-full flex items-center justify-center text-xs font-bold rounded-md transition-colors gap-1 ${
                activeSource === 'mine' ? 'text-white' : 'text-gray-400 hover:text-gray-200'
              }`}
            >
              我的
              {myOCCount > 0 && (
                <span className={`text-[10px] px-1 rounded ${activeSource === 'mine' ? 'bg-white/20' : 'bg-indigo-500/30 text-indigo-300'}`}>
                  {myOCCount}
                </span>
              )}
            </button>
...
const OCGroupCard = React.memo<OCGroupCardProps>(({
  ...
  return (
    <div className="bg-[#1a1b26] border border-gray-800 rounded-xl overflow-hidden hover:border-gray-700 transition-colors">
      <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-transparent border-b border-gray-800">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-300 text-[10px] font-bold rounded-full shrink-0">
              {group.files.length}
            </span>
          </div>
        </div>
...
const OCGalleryCard = React.memo<OCGalleryCardProps>(({
  ...
  return (
    <div
      className="group relative aspect-[832/1216] rounded-xl overflow-hidden bg-gray-800 cursor-pointer border-2 border-gray-800 hover:border-indigo-500/50 transition-all duration-300 hover:shadow-[0_0_20px_rgba(99,102,241,0.3)] hover:scale-[1.02]"
      onClick={onPreview}
    >
      {oc.preview ? (
        <img
