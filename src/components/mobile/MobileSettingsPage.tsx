...
export const MobileSettingsPage: React.FC<MobileSettingsPageProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    if (activeSection === 'profile') fetchProfileStats(profileRange);
  }, [activeSection, profileRange, fetchProfileStats]);
    ...
    switch (activeSection) {
      case 'profile': {
        ...
        const maxValue = profileDailyData.length > 0
          ? Math.max(...profileDailyData.map(d => profileChartMetric === 'calls' ? d.image_calls : d.points_spent), 1)
          : 1;
        return (
          <div className="p-4 space-y-4">
            {/* 用户信息 */}
              ...
              {profileDailyData.length === 0 ? (
                ...
                >
                  <div className="flex items-end gap-2 h-36 pt-6" style={{ minWidth: `${profileDailyData.length * 48}px` }}>
                    {profileDailyData.map((d, i) => {
                      const value = profileChartMetric === 'calls' ? d.image_calls : d.points_spent;
                      const heightPct = maxValue > 0 ? (value / maxValue) * 100 : 0;
                      // 今日模式显示时间段，其他显示日期
                      const dateLabel = profileRange === 'today' ? d.date : d.date.slice(5);
