...
export const AssistantLogList: React.FC<AssistantLogListProps> = ({
  ...
  >
      ...
      agentState.logs.map((log, idx) => {
        ...
        if (log.type === 'success' && log.snapshot) {
          ...
          return (
            <div key={idx} className="animate-slide-in-from-bottom shrink-0 w-full">
              <div
                className={`text-sm px-3 py-2.5 bg-nai-surface text-gray-200 border border-nai-border/50 shadow-sm flex items-start justify-between gap-2 cursor-pointer ${
              ...
              {log.expanded && log.thinking && (
                <div className="text-[10px] px-3 py-2 rounded-b-xl bg-nai-panel-2 text-nai-accent/60 tracking-wider flex items-center border-t border-nai-border">
        ...
        if (log.type === 'error') {
          return (
            <div
              key={idx}
              className="px-3 py-2.5 rounded-xl animate-slide-in-from-bottom bg-nai-surface border border-red-500/30 flex flex-col gap-2 shrink-0 w-full"
            >
          ...
          <div
            ...
            className={`text-sm px-3 py-2 rounded-xl animate-slide-in-from-bottom shrink-0 ${
              log.type === 'user'
                ...
                : 'bg-nai-surface text-gray-300 border border-nai-border self-start max-w-[90%]'
            }`}
          >
    ...
    {isGeneratingPrompt && (
      <div className="text-sm px-3 py-2.5 rounded-xl bg-nai-surface border border-nai-border text-gray-300 animate-fade-in self-start max-w-[85%] shrink-0 flex items-center gap-2">
