...
// ========== Wiki 预览工具函数（与 DesktopChipEditor 共享） ==========
const stepNumericWeight = (weight: number, delta: number): number => Math.round((weight + delta) * 10) / 10;
...
// 可折叠标签类型
// 从原 union 改为 string,允许统一 Tag 管理器动态注册新 subtype
// 旧值 'artist' / 'codex' / 'oc' 仍可继续渲染 (走 LEGACY_TYPE_CONFIG 兜底)
export type CollapsibleTagType = string;

// 兜底配置: 旧 tiptap 节点 type 在 registry 中找不到时使用 (仅 icon + 名称, 颜色全统一 nai-accent)
...
// 统一色: 所有折叠标签共用 nai-accent 系 (用户反馈: 不希望每类不同色)
const UNIFIED_BG = 'bg-nai-accent/25';
const UNIFIED_BORDER = 'border-nai-accent/60';
const UNIFIED_HOVER = 'hover:bg-nai-accent/35';

// 解析 collapsibleTag 节点的 type 字符串到完整视觉配置
// 优先 registry 查询 (含用户自定义), 否则降级到 LEGACY_TYPE_CONFIG, 仍未命中则 'artist' 兜底
// 颜色统一,差异仅在 icon 与名称
...
// 自定义可折叠标签节点
const CollapsibleTagNode = TiptapNode.create({
  name: 'collapsibleTag',
  group: 'inline',
  inline: true,
  atom: true,
...
// 权重高亮扩展——跨段落拼接全文后统一匹配，再映射回文档位置
const WeightHighlightExtension = Extension.create({
      ...
      new Plugin({
        ...
        props: {
          decorations(state) {
            ...
            // 收集全文：拼接所有文本节点，段落边界用 \n 代替
            let fullText = '';
            const segments: { from: number; to: number; start: number }[] = []; // 文档位置 → fullText 位置映射
            ...
            const toDocPos = (offset: number): number => {
              for (let i = segments.length - 1; i >= 0; i--) {
                ...
                if (offset >= seg.start) {
                  return seg.from + (offset - seg.start);
                }
              }
              return 0;
            };

            // 添加高亮装饰，自动裁剪到实际文本节点范围（跳过段落间隙）
            ...
            // Pass 3: 禁用标签（~tag）— 删除线 + 降低不透明度
            const tags = fullText.split(/[,，]/);
            let tagOffset = 0;
            for (const tag of tags) {
              const trimmed = tag.trim();
              if (trimmed.startsWith('~') && trimmed.length > 1) {
                // 找到 ~ 及其后内容在 fullText 中的位置
              ...
              tagOffset += tag.length + 1; // +1 for comma
            }
...
// 辅助函数：将 parentOffset（文档位置偏移）转换为 textContent 中的索引
// atom 节点（如 collapsibleTag）的 nodeSize=1 但 textContent 为空，导致偏移不一致
function docOffsetToTextIndex(parent: Node, docOffset: number): number {
  let textIdx = 0;
  let pos = 0;
...
// 辅助函数：将 textContent 索引转换为 parentOffset（文档位置偏移）
function textIndexToDocOffset(parent: Node, textIndex: number): number {
  let docPos = 0;
  let textPos = 0;
  for (let i = 0; i < parent.childCount; i++) {
    const child = parent.child(i);
...
// 解析值中的 <<type:name:content>> 标记，返回文档 JSON 结构
// type 通用匹配 (artist/codex/oc/scene/other/自定义 subtype)，配合 resolveTagConfig 走 registry 渲染
const COLLAPSIBLE_MARKER_RE = /<<([\w-]+):([^:]+):((?:.|\n)*?)>>/g;

function parseValueToDocContent(value: string): object {
  const text = value || '';
  // 关键: 在整段文本上匹配 marker (content 用 (?:.|\n)*? 已支持跨行),不能先按 \n 拆行——
  // 否则含换行的 marker (如多行角色/场景提示词) 会被换行拆断, 退化成裸文本无法渲染成芯片
  ...
  // 组装 paragraphs: 仅 marker *之外* 的 \n 作为段落边界, marker 内部的换行保留在节点 content 里
...
const PromptEditor = forwardRef<PromptEditorRef, PromptEditorProps>(
  ({ value = '', onChange, onTagsChange, className = '', containerClassName = '', style, placeholder, children, disableCollapsibleTags = false, mobileMode = false, onContentHeightChange }, ref) => {
    ...
    const [tagPanel, setTagPanel] = useState<{
      tag: string;          // 清理后的标签名（空格分隔）
      rawTag: string;       // 原始标签文本（含权重符号）
      translation: string;
      from: number;         // 文档中的绝对位置
      to: number;
      screenX: number;      // 面板显示的屏幕坐标
      screenY: number;
    } | null>(null);
          ...
          >
            ...
            <div className="flex items-start gap-2 px-3 py-2.5">
              <div className="min-w-0 flex-1">
                <div className="font-tag text-[14px] leading-tight text-[#fceda4] truncate" title={tagPanel.tag}>
              ...
              <button className="shrink-0 w-6 h-6 flex items-center justify-center text-white/40 hover:text-white/90 rounded-md hover:bg-white/[0.08] transition-colors" onClick={() => setTagPanel(null)} title="关闭">
                ...
                {weightPresets.map(w => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => { setNumWeight(w); panelActions.setNumericWeight(w); }}
                      title={`${w}::tag::`}
                    >
                      {w}
                    </button>
