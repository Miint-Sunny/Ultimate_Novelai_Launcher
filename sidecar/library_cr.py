...
def create_cr(settings: Settings, data: dict[str, Any]) -> dict[str, Any]:
    ...
    zh_names = data.get("zh_names") or []
    if not isinstance(zh_names, list):
        raise ValueError("zh_names must be a list")
    preview_path = save_base64_asset(settings, "cr", cr_id, data.get("image_base64"))
    row = {
        "id": cr_id,
        "name": name,
        "zh_names_json": json.dumps(zh_names, ensure_ascii=False),
    ...
    try:
        ...
        if preview_path:
            delete_file_if_safe(settings, "cr", str(preview_path))
        raise
...
def update_cr(settings: Settings, cr_id: str, data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not row:
        return None
    zh_names = data.get("zh_names", json.loads(row["zh_names_json"]))
    if not isinstance(zh_names, list):
        raise ValueError("zh_names must be a list")
    updates = {
    ...
    with closing(connect(settings.db_path)) as conn:
        conn.execute(
            """
            UPDATE crs
            SET name = :name, zh_names_json = :zh_names_json, updated_at = :updated_at
            WHERE id = :id
            """,
            updates,
        )
