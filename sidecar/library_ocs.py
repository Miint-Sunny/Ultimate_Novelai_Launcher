...
def create_oc(settings: Settings, data: dict[str, Any]) -> dict[str, Any]:
    ...
    aliases = data.get("zh_aliases") or []
    if not isinstance(aliases, list):
        raise ValueError("zh_aliases must be a list")
    preview_path = save_base64_asset(settings, "oc", oc_id, data.get("preview_base64"))
    row = {
        "id": oc_id,
        "en_name": en_name,
        "zh_name": zh_name,
        "zh_aliases_json": json.dumps(aliases, ensure_ascii=False),
        "tag_group": tag_group,
    ...
    try:
        ...
        if preview_path:
            delete_file_if_safe(settings, "oc", str(preview_path))
        raise
...
def update_oc(settings: Settings, key: str, data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not row:
        return None
    aliases = data.get("zh_aliases", json.loads(row["zh_aliases_json"]))
    if not isinstance(aliases, list):
        raise ValueError("zh_aliases must be a list")
    tag_group = str(data.get("tag_group", row["tag_group"]) or "").strip()
    if not tag_group:
        raise ValueError("tag_group is required")
    updates = {
    ...
    if preview_path and row["preview_path"]:
        delete_file_if_safe(settings, "oc", row["preview_path"])
