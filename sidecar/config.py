...
def _default_data_dir() -> Path:
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "NAI Studio"
    if sys.platform == "win32":
        base = os.environ.get("APPDATA")
        if base:
            return Path(base) / "NAI Studio"
        return Path.home() / "AppData" / "Roaming" / "NAI Studio"
    return Path(os.environ.get("XDG_DATA_HOME", Path.home() / ".local" / "share")) / "nai-studio"
...
class Settings:
    ...
    def db_path(self) -> Path:
        return self.data_dir / "nai-studio.sqlite3"
...
def load_settings() -> Settings:
    data_dir_raw = os.environ.get("NAI_STUDIO_DATA_DIR", "").strip()
    data_dir = Path(data_dir_raw).expanduser() if data_dir_raw else _default_data_dir()
    local = read_local_settings(data_dir)
    port_raw = os.environ.get("NAI_STUDIO_SIDECAR_PORT", "38176").strip()
    env_token = os.environ.get("NAI_TOKEN", "").strip()

    return Settings(
        host=os.environ.get("NAI_STUDIO_SIDECAR_HOST", "127.0.0.1").strip() or "127.0.0.1",
        port=int(port_raw),
        data_dir=data_dir,
        ...
        mock_generation=os.environ.get("NAI_STUDIO_MOCK_GENERATION", "").strip().lower()
        in {"1", "true", "yes", "on"},
    )
