...
logger = logging.getLogger("agent_router.cli")
    ...
    try:
        ...
        for stream in (sys.stdout, sys.stderr):
            try:
                ...
                logger.debug("无法重设控制台流编码", exc_info=True)
        import ctypes
        ...
        try:
            ...
            logger.debug("无法切换 Windows 控制台代码页", exc_info=True)
    except Exception:
        logger.debug("无法启用 Windows ANSI 控制台支持", exc_info=True)
    ...
    async def emit(self, event: SseEvent) -> None:
        ...
        if hasattr(d, "model_dump"):
            try:
                ...
                logger.debug("事件数据不支持 model_dump", exc_info=True)
        d = d or {}
...
def _print_end_node(output, node_ms: int, verbose: bool) -> None:
    ...
    print(
        f"{C.GRY}[{node_ms:>6}ms]{C.R} {C.B}{C.GRN}■ END{C.R}  "
        f"{C.DIM}output type: {type(output).__name__}{C.R}"
    )
...
async def run_repl(
    *,
    ...
    print(
        f"{C.DIM}默认协议: {info['default_protocol']}  "
        f"全局默认 model: {info.get('active_model')}{C.R}"
    )
    ...
    if session_id:
        print(
            f"{C.DIM}Session ID: {session_id[:12]}..."
            f"（用于授权工具调用 /api/oc, /api/artists 等）{C.R}"
        )
    else:
        print(
            f"{C.YEL}Session ID: <空> (search_character 的 OC 部分 / "
            f"search_artist 等需授权工具会降级，传 --session-id 启用){C.R}"
        )
    print(
        f"{C.DIM}Scene: {scene}   model: {model or '(默认)'}   "
        f"verbose: {verbose}   preprocess: {preprocess}{C.R}"
    )
    print(
        f"{C.DIM}命令: /model <名称>  /scene <private|group>  /verbose  "
        f"/preprocess  /clear  /info  /quit{C.R}"
    )
    print()
    ...
    try:
        while True:
            try:
                msg = (await asyncio.to_thread(input, f"{C.YEL}{C.B}You>{C.R} ")).strip()
            ...
            if msg == "/info":
                print(
                    f"{C.DIM}history msgs: {len(history)}  model: {model or '(默认)'}  "
                    f"scene: {scene}  verbose: {verbose}  preprocess: {preprocess}{C.R}\n"
                )
                continue
            if msg == "/verbose":
                verbose = not verbose
            ...
            # ----- 实际调用 chat_agent -----
            print(
                f"{C.DIM}─── run: model={model or '(默认)'} scene={scene} "
                f"history={len(history)} msg={_truncate(msg, 60)!r}{C.R}"
            )
            if env_info:
                first_line = env_info.split("\n", 1)[0]
                print(
                    f"{C.DIM}     env_info 注入: {len(env_info)} 字符"
                    f"（首行: {_truncate(first_line, 60)}）{C.R}"
                )

            tracer = TraceLogger()
            tracer.start()
            print(
                f"{C.GRY}[     0ms      0ms]{C.R} {C.B}{C.BLU}▶ lite + planner "
                f"({model or '默认'}){C.R} start"
            )
            ...
            try:
                ...
                try:
                    ...
                    logger.debug("无法追加 planner 历史", exc_info=True)

                stages = "lite" + ("+planner" if should_draw else "（闲聊收尾）")
                print(
                    f"{C.GRY}[{tracer.elapsed_ms():>6}ms {' ':>8}]{C.R} "
                    f"{C.B}{C.GRN}✓ done{C.R}  total {C.B}{total_ms}ms{C.R}  ({stages})"
                )
                print()
                ...
                if effective_should_draw and effective_specs:
                    for spec in effective_specs:
                        ...
                        if isinstance(chars, list) and chars:
                            ...
                            print(
                                f"{C.YEL}    characters: 类型错误 (期望 list[dict]，"
                                f"得到 {type(chars).__name__}: {chars!r}){C.R}"
                            )
                ...
                if pp_result is not None:
                    try:
                        ...
                        if u:
                            print(
                                f"{C.DIM}  usage(planner): in={u.input_tokens} "
                                f"out={u.output_tokens} reqs={u.requests}{C.R}"
                            )
                    except Exception:
                        logger.debug("无法读取 planner usage", exc_info=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--model", default="", help="初始 model（MODEL_CHOICES 的 key/别名；空=全局默认）"
    )
    parser.add_argument(
        "--base-url",
    ...
    parser.add_argument(
        "--scene", default="private", choices=["private", "group"], help="对话场景（默认 private）"
    )
    parser.add_argument(
        "-v",
        "--verbose",
