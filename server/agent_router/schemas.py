...
# 既给 Gemini/Vertex 一个带 properties 的合法 object schema
# （规避"无 properties 的 OBJECT"被 Vertex 拒），
...
class DrawCharacter(BaseModel):
    ...
    negative: str = Field(default="", description="该角色的反向 tag（可选）")
...
class DrawSpec(BaseModel):
    ...
    positive: str = Field(
        ...
        description=(
            ...
            "再写 white hair, blue eyes, halo, white dress 是错的——"
            "模型已经学过这角色外貌，硬塞反而干扰。\n"
            "2. 用户明确要换装 / cos / 改造时才可加额外服装 tag。"
        ),
    )
    negative: str = Field(default="", description="全局反向 tag（英文逗号分隔，可留空字符串）")
    characters: list[DrawCharacter] = Field(
        default_factory=list,
        description=(
            "多角色场景的分区 tag 列表。每个元素是一个对象，含 "
            "name(角色名 str)、positive(该角色的正向 tag str)、"
            "negative(该角色的反向 tag str，可选)。"
            '示例: [{"name": "芙兰", "positive": "flandre_scarlet, blonde_hair", "negative": ""}]。'
            "**单角色场景必须留空数组 []**，禁止传整数 / 字符串 / 单纯的角色名列表。"
        ),
    )
    size: str | None = Field(
        default=None, description="Portrait / Landscape / Square / Cinemascope"
    )
...
class ChatOutput(BaseModel):
    ...
    reply_text: str = Field(
        ...
        description=(
            "发给用户的**猫娘语气**文本回复"
            "（**必须严格遵循 system prompt 里 [persona] / [reply_rules] 段**）。"
            "无论 assist 还是 create 模式都保留猫娘标签——自称'本喵'、"
            "称用户'主人'、句末用'喵~/喵！/喵。'按情绪选。"
    ...
    draw_spec: dict | None = Field(
        ...
        description=(
            ...
            '      • 单角色场景 → []（禁止 1 / "角色名" / ["角色名"]）\n'
            '      • 多角色场景 → [{"name": str, "positive": str, '
            '"negative"?: str}, ...] **必填**\n'
            "      • 多角色判定：用户原话提到 2+ 专有角色名 / "
            '用户说"分角色"或"分别"或"分角色提示词" /\n'
    ...
    should_draw: bool = Field(default=False, description="是否触发绘图")
    mood: str | None = Field(default=None, description="可选：本轮回复的情绪标签")
...
class LiteResponse(BaseModel):
    ...
    reply_text: str = Field(
        ...
        description=(
            ...
            "should_draw=True 时：'在画了喵~' / '马上来喵。' / '好喵~' "
            "这类一句话告知，不复述用户要画什么。\n"
    ...
    should_draw: bool = Field(
        ...
        description=(
            "是否要触发图片生成。判定规则:\n"
            "- True：用户明确要画图（'画一张'/'再来一张'/'换姿势'/"
            "'继续画'/'加 X tag'/'去掉 X tag'）\n"
...
class AgentResult(BaseModel):
    ...
    thinking: str = Field(default="", description="AI 思考过程 / 闲聊文本")
    positive: str = Field(default="", description="正向 tag")
    negative: str = Field(default="", description="反向 tag")
...
class ChatImage(BaseModel):
    ...
    is_generated: bool = Field(default=False, description="是否是 AI 生成的图（影响历史裁剪策略）")
...
class ChatRequest(BaseModel):
    ...
    user_key: str | None = Field(
        default=None, description="历史 key，如 qq_p_123；不传则按 platform+scene 拼"
    )
    platform: str = Field(default="qq", description="qq / discord / ...")
    scene: Scene = Field(default="private")
    group_id: str | None = Field(default=None)
    text: str = Field(default="", description="用户文本")
    ...
    environment_info: str | None = Field(
        default=None, description="额外环境上下文（用于过渡期复用 main.py 的现有 preprocess）"
    )
...
class ChatResponse(BaseModel):
    ...
    degraded: bool = Field(default=False, description="是否触发了降级（如 prison_break 失败）")
    error: str | None = None
    ...
    image_backend: str = Field(
        default="novelai", description="本轮出图应使用的后端: novelai | anima"
    )
...
class HistoryMessage(BaseModel):
    ...
    is_generated_image: bool = Field(
        default=False,
        validation_alias=AliasChoices("_is_generated_image", "is_generated_image"),
        serialization_alias="_is_generated_image",
    )
...
class DanbooruTag(BaseModel):
    ...
    category: Literal["character", "artist", "copyright", "general", "meta", "unknown"] = Field(
        default="general",
        description=(
            "tag 类别：character=角色, artist=画师, copyright=作品/组合, general=普通, meta=元 tag"
        ),
    )
    post_count: int = Field(default=0, description="Danbooru 上该 tag 的图片数（越多越热门）")
    has_wiki: bool = Field(
        default=False,
